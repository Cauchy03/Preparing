...
另一个是保存，当一个函数返回另一个引用数据类型的时，被外界所接收了就会形成不销毁的作用域，一直存储在堆内存里面，容易造成内存泄漏
...
```
都是改变this指向，唯一的区别就是传入的参数不同，call的第一个参数是函数的this指向，第二个参数往后依次传入函数；而apply的第二个参数是一个数组，包含要传入的所用参数
```

## 13. 连续多个bind，最后this指向是什么？

this永远指向第一次bind绑定的函数

## 14. `es6`新特性？箭头函数和普通函数有啥区别？
...
## 15. `promise.all` 和 `promise.allsettled`区别
...
```
typeof是返回一个字符串，用于检测数据类型
instanceof用于检测构造函数的prototype属性是否出现再某个实例对象的原型链上

instanceof可以判断引用数据类型数据，但不能判断基本数据类型，而且右边的值必须是一个对象否则报错，比如null和undefined

typeof可以判断基本数据类型null除外，引用数据类型只能判断Function
```
...
## 27. splice和slice会改变原数组吗？怎么删除数组最后一个元素？
...
## 30. 解释` requestAnimationFrame/requestIdleCallback`，分别有什么用？
...
```
事件循环是单线程的js在处理异步事件的一个循环过程，首先在执行栈中依次执行同步任务，调用后直接出栈，异步任务则先放到宿主环境中可以是浏览器也可以是node，合适的时机再到回调队列中，等到执行栈为空时，再执行回调队列中的异步任务，这个循环过程叫做事件循环，异步任务又分为微任务和宏任务，微任务始终要比宏任务先执行
```

事件循环（`eventLoop`）是单线程的JavaScript在处理异步事件时进行的一种循环过程，先在执行栈中依次执行同步事件，调用后直接出栈，对于异步事件它会先加入到事件队列中挂起，等主线程空闲时会去执行事件队列中的事件

**事件循环执行过程：**

- 同步代码，调用栈执行后直接出栈
- 异步代码，放到Web API（宿主环境比如浏览器、node）中，等待时机，等合适的时候放入回调队列（callbackQueue），等到执行栈空时eventLoop开始工作，轮询
- **微任务执行时机比宏任务要早**

**Js把任务分为同步任务和异步任务，异步任务又被分为宏任务（宿主环境发起，比如浏览器或node）和微任务（js引擎发起），宏任务和微任务的区别：**

![img](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/26921231cd26436a905314af789d45b2~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.awebp)
...
2. 执行微任务队列中的微任务
3. 尝试DOM渲染
4. 触发Event Loop（事件循环）反复询问callbackQueue（回调队列）中是否有要执行的语句，有则放入call back（执行栈）继续执行
...
- 构造函数模式：用构造函数实例化对象，可以通过函数参数和构造函数体内添加属性和方法创建对象
- 原型模式：使用原型实例指向创建对象的类
- 观察者模式：对象间一对多的依赖关系
- 发布订阅模式：对象间一对多的依赖关系
...
```
Promise是异步编程的一种解决方案，传统的异步操作需要通过回调函数，如果我们需要需要发送发网络请求，根据第一次返回的结果发送第二次请求，一层一层的套下去就会造成回调地狱，代码就会看起来特别臃肿而且耦合度高，容易造成bug不好维护。而Promise就支持链式调用解决回调地狱的问题。
缺点：一旦创建就不能中断；当处于pending状态时，无法得知当前是哪个状态；如果不设置回调函数，Promise内部抛出的错位就不会反应到外部
```
