...
> async 和文档加载并行执行，脚本加载完就立即执行；defer是先加载等到文档解析完再执行
...
**原理：**HTML5 的离线存储是基于一个新建的 `.appcache` 文件的缓存机制(不是存储技术)，通过这个文件上的清单解析离线存储资源，这些资源就会像 cookie 一样被存储了下来。之后当网络在处于离线状态下时，浏览器会通过被离线存储的数据进行页面展示
...
1. **id和class属性**：用于标识HTML元素的唯一性和类别。
2. **src和href属性**：用于指定资源的URL地址，如图片、链接等。
3. **title和alt属性**：用于为图片和链接提供额外的描述信息。
